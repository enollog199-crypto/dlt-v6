from flask import Flask, render_template, request, redirect, url_for, session
import requests, re, random, sqlite3, json, os
import numpy as np
import pandas as pd
from werkzeug.security import generate_password_hash, check_password_hash
from functools import wraps

app = Flask(__name__, template_folder="web")
app.secret_key = "dextro_ai_ui_pro_v63"

def init_db():
    conn = sqlite3.connect("ai.db")
    c = conn.cursor()
    c.execute('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, username TEXT UNIQUE, password TEXT)')
    c.execute('''CREATE TABLE IF NOT EXISTS predict
                 (period TEXT PRIMARY KEY, red TEXT, blue TEXT, hit TEXT, confidence REAL, prob_data TEXT)''')
    conn.commit()
    conn.close()

def autonomous_engine(history, boost=1.0):
    if len(history) < 5:
        return [1,2,3,4,5], [1,2], 0.0, {}, "初始化"
    df = pd.DataFrame(0, index=range(len(history)), columns=range(1,36))
    for i, h in enumerate(history): df.loc[i, h['red']] = 1
    data = df.values[::-1]
    weights = np.exp(np.linspace(-1, 0.5, len(data))) * boost
    prob = np.dot(weights, data) + np.random.normal(0, 0.1, 35)
    conf = round(min(float(np.std(prob) * 15), 99.8), 2)
    red = (np.argsort(prob)[-5:] + 1).tolist()
    blue_df = pd.DataFrame(0, index=range(len(history)), columns=range(1,13))
    for i,h in enumerate(history): blue_df.loc[i, h['blue']] = 1
    blue_prob = np.dot(weights, blue_df.values[::-1]) + np.random.normal(0, 0.1, 12)
    blue = (np.argsort(blue_prob)[-2:] + 1).tolist()
    p_dict = {str(i+1): round(float(prob[i]), 2) for i in range(35)}
    return sorted(red), sorted(blue), conf, p_dict, "多维规律共振中"

def sync_system():
    try:
        headers = {'User-Agent': 'Mozilla/5.0'}
        res = requests.get("https://datachart.500.com/dlt/history/newinc/history.php", timeout=10, headers=headers)
        res.encoding = 'utf-8'
        rows = re.findall(r'<tr class="t_tr1">(.*?)</tr>', res.text, re.S)
        if not rows: return None, []
        history = []
        for row in rows[:30]:
            tds = re.findall(r'<td.*?>(.*?)</td>', row)
            history.append({"period": tds[0], "red": list(map(int, tds[2:7])), "blue": list(map(int, tds[7:9]))})
        conn = sqlite3.connect("ai.db")
        c = conn.cursor()
        for h in history:
            c.execute("UPDATE predict SET hit=? WHERE period=? AND (hit='/' OR hit IS NULL)", 
                      (f"{len(set(json.loads(c.execute('SELECT red FROM predict WHERE period=?',(h['period'],)).fetchone()[0]) if c.execute('SELECT 1 FROM predict WHERE period=?',(h['period'],)).fetchone() else []) & set(h['red']))}+{len(set(json.loads(c.execute('SELECT blue FROM predict WHERE period=?',(h['period'],)).fetchone()[0]) if c.execute('SELECT 1 FROM predict WHERE period=?',(h['period'],)).fetchone() else []) & set(h['blue']))}", h['period']))
        next_p = str(int(history[0]['period']) + 1)
        if not c.execute("SELECT 1 FROM predict WHERE period=?", (next_p,)).fetchone():
            r, b, conf, pdict, exp = autonomous_engine(history)
            c.execute("INSERT INTO predict VALUES (?,?,?,?,?,?)", (next_p, json.dumps(r), json.dumps(b), "/", conf, json.dumps({"prob":pdict,"exp":exp})))
        conn.commit(); conn.close()
        return history[0], history
    except: return None, []

@app.route("/")
def index():
    init_db()
    latest, history = sync_system()
    conn = sqlite3.connect("ai.db")
    rows = conn.execute("SELECT * FROM predict ORDER BY period DESC LIMIT 15").fetchall()
    conn.close()
    records = []
    chart_labels, chart_values = [], []
    for r in rows:
        p_data = json.loads(r[5]) if r[5] else {}
        records.append({"period":r[0],"red":json.loads(r[1]),"blue":json.loads(r[2]),"hit":r[3],"conf":r[4],"exp":p_data.get("exp","")})
        if r[3] != "/":
            chart_labels.append(r[0]); chart_values.append(sum(map(int, r[3].split
