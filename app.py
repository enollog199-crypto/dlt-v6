...
app.secret_key = "dextro_ai_open_v63"
...
def autonomous_engine(history, boost=1.0):
    if len(history) < 15:
        return sorted(random.sample(range(1,36),5)), sorted(random.sample(range(1,13),2)), 50.0, {}, "系统预热"
    ...
    return sorted(red), sorted(blue), confidence, prob_dict, "自主进化中"
...
def index():
    ...
    return render_template("index.html", latest=latest, records=records, top_numbers=top_numbers, chart_data={"labels":chart_labels[::-1],"values":chart_values[::-1]}, logged_in=('user' in session))
